...
# Artifacts produced by open_spreadsheet @istari:extract on this file
EXPECTED_ARTIFACTS = {
    "named_cells.json",
    "worksheet_data.json",
    "workbook.pdf",
...
def test_upload_extract_and_download_artifact(platform: IstariPlatform, tmp_path: Path):
    """Full end-to-end: upload → extract → check artifacts → download one."""
    # Upload
    model = platform.upload_model(
        XLSX_FILE,
    ...
    # Run open_spreadsheet @istari:extract and wait for completion
    job = model.run_job(
        JobDefinition(
    ...
    assert not missing, f"Missing artifacts: {missing}. Got: {artifact_names}"

    # Download named_cells.json and verify it is valid JSON
    import json

    named_cells = job.find_artifact(name="named_cells.json")
