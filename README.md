...
| [`samples/connect/opentext/open-text-extract.ipynb`](samples/connect/opentext/open-text-extract.ipynb) | Official client (`istari_digital_client`) | Open Text: run `@istari:extract` (`textract`) on a Model UUID uploaded in the web app. |
...
| [`samples/workflow-logs/workflow_log_scenario_b.ipynb`](samples/workflow-logs/workflow_log_scenario_b.ipynb) | Official client (`istari_digital_client` + `V3Client`) | External workflow logs — Scenario B: pytest tradespace sweep, winning L-bracket STEP revision, Open CAD extract. Registry Service **> 10.17.3** (2026-05+). `uv sync --group dev --group advanced`. |
| [`samples/version-control/branching-and-change-requests.ipynb`](samples/version-control/branching-and-change-requests.ipynb) | Official client (`istari_digital_client`) | Version control end to end: branch a system, commit files (each commit = a snapshot), open a change request, review the added/removed/changed delta, merge — or close without merging. Client **>= 10.14.0** (07.2026 release). |
